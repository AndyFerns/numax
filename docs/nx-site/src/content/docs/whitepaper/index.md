...
|-------|--------|----------------|
| **nx-core** | *Implemented* | WASM runtime (Wasmtime), sandboxing, host API, SyncManager orchestration |
| **nx-store** | *Implemented* | Persistent local key/value datastore (sled) |
| **nx-sync** | *Implemented* | Pure CRDT data structures, operations and node identity |
...
│  │             │  │ crdt_*      │  │                     │  │
│  └─────────────┘  └──────┬──────┘  └─────────────────────┘  │
│  SyncManager: apply · replication · storage · migration      │
└──────────────────────────┼──────────────────────────────────┘
                           │
          ┌────────────────┼────────────────┐
          ▼                ▼                ▼
   ...
   │   (sled)   │   │ CRDTs +    │   │  TCP + TLS 1.3     │
   │            │   │ operations │   │  (mTLS, allowlist) │
   └────────────┘   └────────────┘   └────────────────────┘
                          ▲
                          │ async runtime (tokio)
                          ▼
...
The host runtime is entirely **async, based on tokio**: WASM module execution, network I/O, store operations and CRDT operation propagation are coordinated by the same asynchronous scheduler. The **SyncManager**, implemented in `nx-core`, is the junction point between CRDT host API, local store and network: it receives the operations generated by the guest, materializes them on sled and propagates them to the active peers.
...
In `v0.1.1`, the former monolithic implementation was split under
`nx-core/src/sync_manager/` into focused modules for orchestration, operation
application, replication, storage, peer health, schema handling and migration.
This is an architectural cleanup: replication behavior remains covered by the
...
|------|-------------|--------|
| PNCounter | Counter with increments and decrements | *Implemented* |
